...
{"PIC16C84",	PIC16C84,	1024,	64,	0,	DS30262E,  0x2000, 1, 0, 1,	1},
...
int pic14_index = (sizeof(pic14_map) / sizeof(struct pic14_dsmap)) - 1;
...
pic14_selector(struct k8048 *k)
{
	...
	for (i = 0; pic14_map[i].deviceid; ++i) {
                if (i % 4 == 3)
                        printf("%s\n", pic14_map[i].devicename);
                else
                        printf("%s\t", pic14_map[i].devicename);
        }
        if (i % 4)
                printf("\n");
}
...
pic14_write_word(struct k8048 *k)
{
	...
	case DS41573C:  /* PIC16F1507 */
        case DS41439A:  /* PIC16F1847 */
	case DS41397B:	/* PIC16F1936           BEGIN PROGRAMMING INTERNAL */
...
pic14_dumpdevice(struct k8048 *k)
{
	...
	/* USERID */
	for (i = 0; i < 4; ++i) {
		if (pic14_conf.index[i] != MASK14BIT) {
			if (header == 0) {
				if (pic14_map[pic14_index].configaddr >= 0x8000) {
					/* Extended address = 0x0001 */
	...
	for (i = 0; i < pic14_map[pic14_index].nconfig; ++i) {
		if (pic14_conf.index[PIC14_CONFIG_WORD1 + i] != MASK14BIT) {
			if (header == 0) {
				if (pic14_map[pic14_index].configaddr >= 0x8000) {
					/* Extended address = 0x0001 */
					printf(":020000040001F9\n");
